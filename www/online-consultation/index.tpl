...
<div id="body">
					...
					<div class="content wrap tbl_calendar mb50">
						<p class="pos_ac fw_bold mb10 month">
							{if $mst_calendar.back_week|date_format:'%Y-%m-%d' >= date("Y-m-d")}
								<a href="./index.php?y={$mst_calendar.back_week|date_format:"%Y"}&m={$mst_calendar.back_week|date_format:"%m"}&d={$mst_calendar.back_week|date_format:"%d"}">&lt;&lt;</a>
							{else if $mst_calendar.display_date|date_format:'%Y-%m-%d' == date("Y-m-d")}
								<span>&lt;&lt;</span>
							{else}
								<a href="./index.php?y={date("Y")}&m={date("m")}&d={date("d")}">&lt;&lt;</a>
							{/if}
	...
	{else}
							{if $mst_calendar.next_week|date_format:'%Y-%m-%d' >= date("Y-m-d") && $mst_calendar.next_week|date_format:'%Y-%m-%d' < $mst_calendar.next_limit|date_format:'%Y-%m-%d'}
								<a href="./index.php?y={$mst_calendar.next_week|date_format:"%Y"}&m={$mst_calendar.next_week|date_format:"%m"}&d={$mst_calendar.next_week|date_format:"%d"}">&gt;&gt;</a>
							{else}
								<span>&gt;&gt;</span>
							{/if}
	{/if}
											...
											{if !empty($calendar.day)}
												...
												{else}
													{if $calendar.resept.$teacher_id.$time_id|default:"" == 1}
														<td><a href="./form.php?y={$calendar.day|substr:0:4}&m={$calendar.day|substr:5:2}&d={$calendar.day|substr:8:2}&t={$time_id}&teacher={$teacher_id}">〇</a></td>
													{elseif $calendar.resept.$teacher_id.$time_id|default:"" == 2}
														<td>-</td>
													{else}
														<td>-</td>
													{/if}
