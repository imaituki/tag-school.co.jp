					...
					<table class="tbl_form bg0">
						<tbody>
							...
							<tr>
								...
								<td>
									{if !empty($message.ng.school_type)}<span class="error">※{$message.ng.school_type}</span>{/if}
									<select name="school_type">
										<option value="">選択してください。</option>
										{html_options options=$OptionSchoolType selected=$arr_post.school_type}
									</select>
									{if !empty($message.ng.school)}<span class="error">※{$message.ng.school}</span>{/if}
									<input type="text" name="school" value="{$arr_post.school|default:''}" />
								</td>
							</tr>
							<tr>
