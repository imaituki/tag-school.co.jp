...
class AD_online_consultation2 {
	...
	function check( &$arrVal, $mode ) {
		...
		if(!empty($arrVal["teacher"])){
			$where_teacher = " AND teacher = '" . $arrVal["teacher"] . "'";
		}

		// 予約数上限チェック（INSERT時）
		if( empty( $arrVal["cancel_flg"] ) ) {
			$chk = $this->_DBconn->selectCtrl( array( "select" => "*", "from" => $this->_CtrTable, "where" => "delete_flg = 0 AND cancel_flg = 0 AND date =  '"  . $arrVal["date"] . "' AND time = '" . $arrVal["time"] . "'" . $where_teacher, "order"  => "date ASC" ), array( "fetch" => _DB_FETCH_ALL ) );

			if( !empty($arrVal["teacher"]) && count( $chk ) >= 1 && strcmp( $mode, "insert" ) == 0 ) {
				if( empty($res["ng"]["teacher"] ) ){
					$res["ng"]["teacher"] = NULL;
				}
				$res["ng"]["teacher"] .= "指定日時は既に予約上限に達しています。<br />";
			}
		}
	...
	function GetSearchCalendarList( $search ) {
		...
		if( !empty( $search["search_teacher"] ) ) {
			$creation_kit["where"] .= "AND " . $this->_CtrTable . ".teacher = '". $search["search_teacher"] . "' ";
		}
