...
function smarty_function_html_select_time($params, $template)
{
    ...
    foreach ($params as $_key => $_value) {
        switch ($_key) {
            case 'time':
			    if (preg_match('/^(\d{0,4}\/\d{0,2}\/\d{0,2}) (\d{0,2}:\d{0,2}:\d{0,2})$/', $_value, $found)) {
			        $time = str_replace( ":", "-", $found[2] );
			    }elseif (preg_match('/^(\d{0,4}-\d{0,2}-\d{0,2}) (\d{0,2}:\d{0,2}:\d{0,2})$/', $_value, $found)) {
			        $time = str_replace( ":", "-", $found[2] );
			    }elseif (preg_match('/^(\d{0,4}-\d{0,2}-\d{0,2})/', $_value, $found)) {
			        $time = $found[1];
			    }elseif (preg_match('/(\d{0,2}:\d{0,2}:\d{0,2})$/', $_value, $found)) {
			        $time = $found[1];
                }elseif (!is_array($_value) && $_value !== null) {
    ...
    if (isset($params['time']) && is_array($params['time'])) {
    	...
    	if( is_numeric( $time ) ) {
	        ...
	        list($_hour, $_minute, $_second) = $time = explode('-', $time);
	    }
    }
    ...
    if ($display_hours) {
        ...
        $hour_start = $use_24_hours ? ( ( empty( $hour_start ) ) ? 0 : $hour_start ) : 1;
        $hour_end = $use_24_hours ? ( ( empty( $hour_end ) ) ? 23 : $hour_end ) : 12;
        for ($i=$start; $i <= $end; $i++) {
        	if( $hour_start <= $i && $i <= $hour_end ) {
	            $_val = sprintf('%02d', $i);
    ...
    if ($display_minutes) {
        ...
        if (isset($minute_empty) || isset($all_empty) && $_minute == "" ) {
        	$selected = null;
        }
    ...
    if ($display_seconds) {
        ...
        if (isset($second_empty) || isset($all_empty) && $_second == "" ) {
        	$selected = null;
        }
