...
$(function () {
	...
	$('.tab_navi a').on('click', function () {
		...
		$(".course_navi").removeClass('active');
	...
	$('.tab_navi2 a').on('click', function () {
		tsp = $(this).parents('.tab_navi2');
		tse = $(tsp).attr('data-tab');
		ttg = $(this).attr('href');
		tflg = 1;
		if ($(this).parents('.tab2').hasClass('active')) { tflg = 0 }

		$(tsp).find('.tab2').removeClass('active');
		$('.' + tse).removeClass('active');
		if (tflg == "1") {
			$(ttg).addClass('active');
