...
<div id="calendar" class="wrapper wrapper-content">
	<p class="calendar_date_select pos_ac large f-bold mb30">
		<span>
			<a href="./index.php?y={$mst_calendar.back_date|date_format:"%Y"}&m={$mst_calendar.back_date|date_format:"%m"}{if !empty($arr_post.search_teacher)}&teacher={$arr_post.search_teacher}{/if}" class="btn btn-primary btn-s" style="color:#FFF; text-decoration: none; font-weight: bold;">&lt;<span class="hidden-xs"> 前月へ</span></a>
			&nbsp;
			...
			&nbsp;
			<span class="hidden-xs">&nbsp;&nbsp;&nbsp;&nbsp;</span>
			<a href="./index.php?y={$mst_calendar.next_date|date_format:"%Y"}&m={$mst_calendar.next_date|date_format:"%m"}{if !empty($arr_post.search_teacher)}&teacher={$arr_post.search_teacher}{/if}" class="btn btn-primary btn-s" style="color:#FFF; text-decoration: none; font-weight: bold;"><span class="hidden-xs">次月へ </span>&gt;</a>
		</span>
	</p>
	<div align="center">
				...
				<td class="pos_vt pointer" >
					...
					{foreach from=$calendar.calendar item="detail" key="key2" name="loopDetail"}
						<div class="{if $detail.cancel_flg == 1}cancelday{elseif $detail.mail=="office@web3.co.jp"}reserveday _admin{else}reserveday{/if}">
							<a href="./edit.php?id={$detail.id_online_consultation}"><span class="reserve-time">{$OptionReserveTime[$detail.time]}</span>&nbsp;{$detail.name2}{if !empty($OptionTeacher[$detail.teacher])}<br>担当：{$OptionTeacher[$detail.teacher]}{/if}{if $detail.cancel_flg == 1}<br>※キャンセル{/if}</a>
						</div>
					{/foreach}
				</td>
			{else}
