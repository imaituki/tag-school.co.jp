 ...
 * @return string
 */
function smarty_function_html_select_date2($params, $template)
{
    ...
    foreach ($params as $_key => $_value) {
        switch ($_key) {
            case 'time':
			    if (preg_match('/^(\d{0,4}-\d{0,2}-\d{0,2})/', $_value, $found)) {
			        $time = $found[1];
                }elseif (!is_array($_value) && $_value !== null) {
                    $time = strftime( '%Y-%m-%d', smarty_make_timestamp($_value) );
                }
                break;
    ...
    if (isset($params['time']) && is_array($params['time'])) {
        ...
        list($_year, $_month, $_day) = $time = explode('-', $time);
    }
    ...
    if( $order_format == null ){
        ...
        $order_format = str_replace('%Y','%1$s',$order_format);
        $order_format = str_replace('%M','%2$s',$order_format);
        $order_format = str_replace('%D','%3$s',$order_format);
        $_html = sprintf( $order_format, $_html_years ,$_html_months , $_html_days );
    }
