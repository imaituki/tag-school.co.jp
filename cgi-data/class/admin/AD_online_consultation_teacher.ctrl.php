...
//  クラス
//-------------------------------------------------------
class AD_online_consultation_teacher {
	...
	function check( $arrVal, $mode ) {
		...
		$objInputCheck->entryData( "URL", "url", $arrVal["url"], array( "CHECK_MIN_MAX_LEN" ), 0, 255 );
		$objInputCheck->entryData( "表示／非表示", "display_flg", $arrVal["display_flg"], array( "CHECK_EMPTY", "CHECK_MIN_MAX_NUM" ), 0, 1 );

		// チェックエントリー（UPDATE時）
		if( ( strcmp( $mode, "update" ) == 0 ) ) {
			$objInputCheck->entryData( "講師ID", "all", $arrVal[$this->_CtrTablePk], array( "CHECK_EMPTY", "CHECK_NUM" ), null, null );
		}
