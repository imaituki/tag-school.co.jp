...
$objMember = new FT_member( $objManage, $_ARR_MAIL["unsubscribe"]["savePath"] );
...
if( empty( $message["ng"] ) ) {
	...
	// 登録処理
	$res = $objMember->delete( $_COOKIE["mem_id_member"] );
...
if( empty( $message["ng"] ) ) {
	...
	// メールアドレス設定
	$_SESSION["front"]["mypage"]["unsubscribe"]["POST"]["mail"] = $arr_post["mail"];
