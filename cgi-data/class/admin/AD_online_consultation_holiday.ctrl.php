...
//  クラス
//-------------------------------------------------------
class AD_online_consultation_holiday {
	...
	function GetSearchList( $search, $option = null ) {
		...
		// SQL配列
		$creation_kit = array(  "select" => "id_online_consultation_holiday, DATE_FORMAT( date, '%Y%m%d' ) as date",
								"from"   => $this->_CtrTable,
								"where"  => "date >= '" . $search["date1"] . "' AND date <= '" . $search["date2"] . "' ",
								"order"  => "date ASC" );
